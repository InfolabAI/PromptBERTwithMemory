...
save_path = "saved_bert-base-multilingual-cased2"
...
config = BertConfig.from_pretrained(save_path, output_hidden_states=True)
model = BertModel.from_pretrained(save_path, config=config).to("cuda")
...
train_df2, test_df = Dataset_U.split_df_train_val(train_df2, train_ratio=0.9)
...
uniformaly_instance.anomaly_scorer.load(faiss_path)
